...
# LoRA adapter. `git lfs pull` + size check: a plain clone without LFS leaves a
# ~130-byte pointer file that only explodes later at model load time.
ARG LORA_REPO=https://github.com/da03bod-cpu/qwen3-nonprofit-training.git
ARG LORA_BRANCH=main
RUN git clone --depth 1 --branch "${LORA_BRANCH}" "${LORA_REPO}" /tmp/qwen3-training && \
    cd /tmp/qwen3-training && git lfs pull && \
    test -f lora-release/adapter_config.json && \
    test "$(stat -c %s lora-release/adapter_model.safetensors)" -gt 1048576 && \
    mkdir -p /app/lora-release && cp -a lora-release/. /app/lora-release/ && \

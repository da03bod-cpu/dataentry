...
program JSON using **base Qwen3-8B without LoRA** (PaddleOCR-VL only for scanned PDF pages).
...
## Strict grounded extraction (v2)

This build intentionally uses the base Qwen model **without the nonprofit LoRA**.
The model proposes candidates, then `pipeline/grounding.py` rejects any record whose
name cannot be found in the extracted document and nulls unsupported fields.

For deployment debugging, call `type=extract_text`. The response includes:
- `version`: expected `strict-grounded-v2`
- `sha256`: hash of the actual text received/extracted by the worker

If two different uploads produce the same `sha256`, the problem is before the model
(e.g. n8n is sending stale/pinned binary data). If `version` is not the expected value,
the Serverless endpoint is still running an older container image.

`MAX_INPUT_CHARS=0` means `program_extract` is not globally truncated; it is still
processed in Qwen chunks controlled by `QWEN_CHUNK_CHARS`.

...
def _check_lora(path):
    weights = os.path.join(path, "adapter_model.safetensors")
    config = os.path.join(path, "adapter_config.json")
    if not os.path.isfile(config) or not os.path.isfile(weights):
        raise RuntimeError(f"LoRA adapter not found in {path} (need adapter_config.json + adapter_model.safetensors).")
    if os.path.getsize(weights) < 1024 * 1024:
        raise RuntimeError(
            f"{weights} is only {os.path.getsize(weights)} bytes: it is a Git LFS pointer, not the real weights. "
            "Rebuild the image with git-lfs (git lfs pull)."
        )
...
def load_model():
    ...
    with _lock:
        ...
        _check_lora(LORA_PATH)

        log.info("Loading %s (4bit=%s) + LoRA %s ...", QWEN_MODEL_NAME, LOAD_IN_4BIT, LORA_PATH)
        ...
        base = AutoModelForCausalLM.from_pretrained(QWEN_MODEL_NAME, **kwargs)
...
def split_document(text, chunk_chars=QWEN_CHUNK_CHARS):
    ...
    for chunk in chunks:
        ...
        for line in chunk.splitlines():
            while len(line) > chunk_chars:  # a single giant line
                if buf:
                    final.append(buf)
                    buf = ""
...
def _generate_one(document_text, chunk_index, total_chunks, max_new_tokens):
    ...
    else:  # silence Qwen's sampling defaults from generation_config.json
...
def generate_program_items(document_text, max_new_tokens=None, chunk_chars=None):
    """Run Qwen over the document. Returns (raw_items, meta).

    A failing chunk is logged and skipped; only if EVERY chunk fails do we raise."""

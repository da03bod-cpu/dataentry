...
def collect_sources(req):
    ...
    if len(sections) == 1 and not req.text:
        document = sections[0][1]  # single file: keep the extracted layout unchanged
    else:
...
def _truncate(document, limit, warnings):
    if not limit or limit <= 0 or len(document) <= limit:
        return document
...
def _run_model(document, max_new_tokens, chunk_chars):
    ...
    cleaned = clean_programs(items)
    if STRICT_GROUNDING:
        cleaned, grounding_meta = ground_programs(cleaned, document)
        meta.update(grounding_meta)
    return merge_programs(cleaned), meta
...
def run_program_autofill(req):
    ...
    if programs:
        ...
        # Strict mode: never invent a filename/program name as a fallback.
        best = {k: None for k in FIELDS}
        extra_notes.insert(0, "لم يتم العثور على برنامج/مشروع مؤكد في النص المستخرج؛ لم تتم تعبئة أي قيمة تخمينية.")
